...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (position == mChat.size()-1){
            if (chat.isIsseen()) {
                holder.txt_seen.setBackground(mContext.getResources().getDrawable(R.drawable.ic_done_all));
            } else {
                holder.txt_seen.setBackground(mContext.getDrawable(R.drawable.ic_done));
            }
        } else {
